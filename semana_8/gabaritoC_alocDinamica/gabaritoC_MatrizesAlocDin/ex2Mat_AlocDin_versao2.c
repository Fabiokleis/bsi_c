...
// essa versao considera a alocacao de uma matriz triangular inferior
int * alocaLinha(int tam);
int ** alocaMatrizTriangInf(int tamL, int tamMaxC);
...
int * alocaLinha(int tam)
{
    ...
    if(v==NULL)
    {
        printf("Erro alocacao linha da matriz!!!");
        exit(1);
    }


}
int ** alocaMatrizTriangInf(int tamL, int tamC)
{
    int i, **m;
    ...
    for(i=0;i<tamL;i++)
        m[i]= alocaLinha(i+1); //cada linha vai ser alocada com um tamanho diferente  de 1 a tam
    return m;
}
void desalocaMatriz(int **m, int tamL)
{
